...
function checkHeaders(){
	...
	if (isset($GLOBALS['headers']['X-Csrf-Token'])&&!empty($GLOBALS['headers']['X-Csrf-Token'])&&isset($GLOBALS['headers']['RCPTEQ'])&&!empty($GLOBALS['headers']['RCPTEQ'])){
		...
		&&$GLOBALS['headers']['RCPTEQ']=='bUIxNOWYDHALHUITyNjMyMUY0') {
			return true;
		}
	}
	return false;
...
function checkArguments(){
	...
	&&isset($_POST['3fc10ee46d843e80074879c625e74e4ab87925190deaf3d2df41c1480941afa9'])&&!empty($_POST['3fc10ee46d843e80074879c625e74e4ab87925190deaf3d2df41c1480941afa9'])
	&&isset($_POST['3fc10ee46d84367890879c625e74e4ab87925190deaf3d2df41c251d9a4afa9'])&&!empty($_POST['3fc10ee46d84367890879c625e74e4ab87925190deaf3d2df41c251d9a4afa9'])) {
		...
		$GLOBALS['args']['parent_no']=dataCleaner($_POST['3fc10ee46d843e80074879c625e74e4ab87925190deaf3d2df41c1480941afa9']);
		$GLOBALS['args']['phone']=dataCleaner($_POST['3fc10ee46d84367890879c625e74e4ab87925190deaf3d2df41c251d9a4afa9']);
	}else{
		$argsMissing=true;
	}
	return !$argsMissing;
...
function acceptOutingRequest(){
	require $_SERVER['DOCUMENT_ROOT']."api/warden/dbusers/outing_manager_warden.php";
	$stmtgetOutingUser->execute();
	$stmtgetOutingUser->setFetchMode(PDO::FETCH_ASSOC);
	$result=$stmtgetOutingUser->fetchAll();
	$hostelerUserId="";
	if (count($result)>0) {
		$hostelerUserId=$result[0]['user_id'];
		if($result[0]['status']=="1"){
			if ($stmtAcceptOuting->execute()) {
				if ($stmtUpdateHostelerAccepted->execute()) {
					$con=null;
					return TRUE;
				}
			}
		}else{
			$GLOBALS['response']['message']="invalid status";
			return FALSE;
		}
	}else{
