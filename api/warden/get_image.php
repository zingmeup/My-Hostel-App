...
function checkHeaders(){
  ...
  if (isset($GLOBALS['headers']['X-Csrf-Token'])&&!empty($GLOBALS['headers']['X-Csrf-Token'])&&isset($GLOBALS['headers']['RCPTEQ'])&&!empty($GLOBALS['headers']['RCPTEQ'])){
    ...
    &&$GLOBALS['headers']['RCPTEQ']=='bUIxNmjUTgyNjMyMUY0') {
      return true;
    }
  }
  return false;
...
&&isset($_POST['52460ee46d843e80074879c625e74e4f19147c5b9c1af3d2df41c2518ad3ecff'])&&!empty($_POST['52460ee46d843e80074879c625e74e4f19147c5b9c1af3d2df41c2518ad3ecff'])) {
    ...
    $GLOBALS['args']['hash']=dataCleaner($_POST['52460ee46d843e80074879c625e74e4f19147c5b9c1af3d2df41c2518ad3ecff']);
  }else{
    $argsMissing=true;
  }
  return !$argsMissing;
...
function getImageForHosteler(){
  require $_SERVER['DOCUMENT_ROOT']."/api/warden/dbusers/hosteler_image_manager.php";
  $stmtgetDetails->execute();
  $stmtgetDetails->setFetchMode(PDO::FETCH_ASSOC);
  $result=$stmtgetDetails->fetchAll();
  $con=null;
  $GLOBALS['response']['hosteler_image']=$result[0];
}
